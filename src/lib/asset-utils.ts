...
export const getMockInstructions = (type?: AssetType): string[] => {
    ...
    switch (type) {
      ...
      case 'Ubuntu Server':
        rawInstructions = [
          ...
          "   - Example commands (replace version if needed):\n" +
          "     ```yaml\n" +
          ...
          "   - Add the generated YAML snippet (from the Configuration tab or Wizard) to your `prometheus.yml` file under `scrape_configs:`. Ensure `<YOUR_SERVER_IP>` is replaced with the actual IP from the Wizard's configuration (param1).",
  ...
  return rawInstructions.map((stepContent, index) => {
    ...
    const formattedContent = formatInstructionStep(stepContent); 
